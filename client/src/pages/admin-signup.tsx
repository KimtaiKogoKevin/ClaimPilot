...
import {
  Eye, EyeOff, Shield, Mail, Lock, User, Building2,
  FileCheck, Users, BarChart3, CheckCircle, ArrowRight, Clock
} from "lucide-react";
...
const adminSignupSchema = z.object({
  ...
  companyType: z.enum(['insurance_company', 'broker', 'other'], { required_error: "Please select a company type" }),
...
const PERKS = [
  { icon: FileCheck, text: "Review and approve claims" },
  { icon: BarChart3, text: "Analytics and insights dashboard" },
  { icon: Users,    text: "User and team management" },
  { icon: Shield,   text: "Enterprise-grade security" },
];

const STEPS = [
  "Our team verifies your business registration",
...
export default function AdminSignupPage() {
  ...
  const form = useForm<AdminSignupForm>({
    ...
    defaultValues: {
      ...
      companyType: undefined, businessRegistrationNumber: "",
      phoneNumber: "", address: "", reason: "",
    }
  });

  const signupMutation = useMutation({
    mutationFn: async (data: AdminSignupForm) => {
      const res = await apiRequest("POST", "/api/auth/signup/admin", data);
      return res.json();
    },
    onSuccess: () => {
      setApplicationSubmitted(true);
      toast({ title: "Application submitted", description: "Your admin access request is pending review." });
    },
    onError: (error: any) => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background p-6">
        <div className="w-full max-w-lg text-center animate-fade-in">
          ...
          <p className="text-muted-foreground mb-8">Your admin access request is pending approval.</p>

          <div className="bg-slate-50 border border-border rounded-2xl p-6 text-left mb-6">
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-2/5 flex-col justify-between bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 text-white p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-8">
          <div>
            <div className="inline-flex items-center gap-2 bg-amber-500/20 border border-amber-500/30 text-amber-200 rounded-full px-3 py-1 text-xs font-semibold mb-4">
            ...
            <p className="text-slate-300 text-base leading-relaxed">
              Access powerful tools for claims management, AI damage assessment, and analytics.
            </p>
          </div>
          <ul className="space-y-3">
          ...
          <Alert className="bg-white/10 border-white/20 text-slate-200 text-sm">
            <AlertDescription className="text-slate-200">
              Admin accounts require business verification. Our team reviews credentials before granting access.
            </AlertDescription>
          </Alert>
        </div>
      ...
      {/* Right form panel */}
      <div className="flex-1 flex flex-col justify-start px-6 py-12 bg-background overflow-y-auto">
        <div className="w-full max-w-sm mx-auto">
          ...
          <h2 className="text-2xl font-bold text-foreground mb-1">Apply for admin access</h2>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(d => signupMutation.mutate(d))} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormField control={form.control} name="firstName" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input data-testid="input-first-name" placeholder="John" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="lastName" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input data-testid="input-last-name" placeholder="Doe" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
              </div>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <div className="relative">
                      ...
                      <Input data-testid="input-email" className="pl-10" placeholder="john@company.com" type="email" {...field} />
                    </div>
                  </FormControl>
                  <FormMessage />
                </FormItem>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <div className="relative">
                      ...
                      <Input data-testid="input-company-name" className="pl-10" placeholder="ABC Insurance Ltd" {...field} />
                    </div>
                  </FormControl>
                  <FormMessage />
                </FormItem>
              ...
              <FormField control={form.control} name="businessRegistrationNumber" render={({ field }) => (
                <FormItem>
                  <FormLabel>Registration number <span className="text-muted-foreground font-normal">(optional)</span></FormLabel>
                  <FormControl><Input data-testid="input-registration-number" placeholder="BN-123456789" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="phoneNumber" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input data-testid="input-phone-number" placeholder="+1 (555) 000-0000" type="tel" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="address" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input data-testid="input-address" placeholder="123 Business St, City" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="reason" render={({ field }) => (
                <FormItem>
                  <FormLabel>Reason for access <span className="text-muted-foreground font-normal">(optional)</span></FormLabel>
                  <FormControl>
                    <Textarea data-testid="textarea-reason" placeholder="Tell us how you plan to use the platform…" className="min-h-[80px] resize-none" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )} />

              <Button type="submit" className="w-full gap-2 mt-2" disabled={signupMutation.isPending} data-testid="button-submit-application">
              ...
              <p className="text-xs text-center text-muted-foreground pt-2">
                A policy holder?{" "}
                <Link href="/auth/insured" className="text-primary hover:underline" data-testid="link-insured-signup">
